...
const EventForm = ({userId, type}: EventFormProps) => {
    ...
    const form = useForm<z.infer<typeof eventFormSchema>>({
      ...
      if (type === 'Create'){
        try {
            const newEvent = await createEvent({event: { ...values, imageUrl: uploadedImageUrl},
            userId,
            path: '/profile'
        })
    ...
    <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-5">
            ...
            render={({field}) => (
                <FormItem className="w-full">
                    <FormControl>
                        <div className="flex-center h-[54px] w-full overflow-hidden rounded-full bg-grey-5 px-4 py-2">
